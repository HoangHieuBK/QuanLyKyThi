...
public interface ExamAnswerRepository extends JpaRepository<Exam_Answer, Integer>{
	...
	@Query(value = "select  ea.question_id,ea.choose_answer from exam_answer ea where exam_user_id=:exam_user_id", nativeQuery=true)
	List<Object> getListQuestionByExamUserId(@Param("exam_user_id") int exam_user_id);
